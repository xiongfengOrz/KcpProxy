...
static struct kcp_config config;
...
static void kcp_param_init(struct kcp_param *param)
{
	memset(&config.param, 0, sizeof(struct kcp_param));
...
void config_init()
{
	...
	kcp_param_init(&config.param);
}

#if 0
void kcp_param_free(struct kcp_param *param)
{
...
{
	return kcp_parse_json_param(&config.param, filename);
}

// 1: error; 0, success
int kcp_parse_json_param(struct kcp_param *param, const char *filename)
{
	if (!param)
		return 1;
